...
from fastapi import APIRouter, Body, HTTPException, Header, Path, Query, Request, Response, status
...
class HTTPExceptionModel(BaseModel):
    detail: str = Field(description="Error message", example="Movie not found")
...
def get_movie(request: Request, response: Response, id: int = Path(description="Movie id"), my_special_header: str = Header(description="Special header")):
    log.info(f"special header {my_special_header} rest {request.headers}")
    if id not in movies:
        raise HTTPException(detail="Movie id not found", status_code=status.HTTP_404_NOT_FOUND)
    response.headers["return-header"] = "return-value-header"
    return movies[id]
